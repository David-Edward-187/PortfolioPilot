  ...
  return (
    <>
      <div className="py-16 md:py-20">
        ...
        <div className="max-w-5xl mx-auto grid md:grid-cols-2 gap-12 md:gap-20 items-start bg-card/80 border-border/50 backdrop-blur-md p-8 md:p-12 rounded-lg shadow-2xl">
          <div className="space-y-7 animate-fadeIn" style={{animationDelay: '0.1s'}}>
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              <div>
                ...
                <Input id="name" {...form.register("name")} placeholder="Your Name" className="bg-input border-input-border focus:border-primary focus:ring-primary text-base py-3" />
              ...
              <div>
                ...
                <Input id="email" type="email" {...form.register("email")} placeholder="your.email@example.com" className="bg-input border-input-border focus:border-primary focus:ring-primary text-base py-3" />
               ...
               <div>
                ...
                <Input id="subject" {...form.register("subject")} placeholder="Regarding..." className="bg-input border-input-border focus:border-primary focus:ring-primary text-base py-3" />
              ...
              <div>
                ...
                <Textarea id="message" {...form.register("message")} placeholder="Your message here..." rows={5} className="bg-input border-input-border focus:border-primary focus:ring-primary text-base min-h-[140px]" />
                {form.formState.errors.message && <p className="text-xs text-destructive mt-1.5">{form.formState.errors.message.message}</p>}
              </div>
              <Button type="submit" className="w-full bg-primary text-primary-foreground hover:bg-accent hover:text-accent-foreground py-3.5 text-lg shadow-lg hover:shadow-primary/50 transform hover:scale-105 transition-transform duration-300 group active:scale-95" disabled={isLoading}>
                {isLoading ? (
            ...
            <div className="flex space-x-8 pt-4">
              {socialLinksData.map((link) => (
                <a
                  ...
                  className={`text-muted-foreground p-3.5 rounded-full hover:bg-primary/15 transform transition-all duration-300 hover:scale-110 active:scale-95 focus-visible:ring-2 ring-ring ring-offset-background ${link.color}`}
                >
      ...
      <Dialog open={showSuccessModal} onOpenChange={setShowSuccessModal}>
        <DialogContent className="sm:max-w-md p-7 bg-card rounded-lg shadow-xl border-border/70">
          <DialogHeader className="text-center">
            <div className="mx-auto flex items-center justify-center h-16 w-16 rounded-full bg-primary/15 mb-5 border-2 border-primary">
               <svg className="h-10 w-10 text-primary" fill="none" viewBox="0 0 24 24" strokeWidth="2.5" stroke="currentColor">
          ...
          <div className="mt-8 flex justify-center">
            <Button onClick={() => setShowSuccessModal(false)} className="bg-primary text-primary-foreground hover:bg-accent hover:text-accent-foreground text-base px-8 py-3 shadow-md hover:shadow-lg">
              Close
            </Button>
          </div>
        </DialogContent>
